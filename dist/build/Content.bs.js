  ...
  var require_Content_bs = __commonJS((exports) => {
    ...
    function render(state) {
      ...
      if (typeof time === "number") {
        if (time !== 0) {
          return removeOverlay(void 0);
        } else {
          return applyOverlay("BLOCKED");
        }
      } else {
        return applyOverlay("Delayed: " + String(time._0));
      }
    }
    function onMessage(f) {
      chrome.runtime.onMessage.addListener(function(message, _sender) {
        return Curry._1(f, message);
      });
    }
    function sendMessage(prim) {
      chrome.runtime.sendMessage(prim);
    }
    var Foreground = App.CreateForeground({
      onStateChange: render,
      sendMessage,
      onMessage
    });
    var watchUrlChange = setInterval(function(param) {
      if (Foreground.state.contents.url !== window.location.href) {
        return Curry._1(Foreground.reduce, {
          TAG: 0,
          _0: window.location.href
        });
      }
    }, 1e3);
    Curry._1(Foreground.reduce, 1);
    window.onfocus = function(param) {
      return Curry._1(Foreground.reduce, 1);
    };
    window.onblur = function(param) {
